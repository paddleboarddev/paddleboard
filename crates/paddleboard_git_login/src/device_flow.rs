//! GitHub OAuth Device Flow (RFC 8628) — "OAuth lite".
//!
...
//!
//! This module is transport + parsing only; the UI (`git_ui`'s login modal)
//! drives the poll loop on its own executor and stores the resulting token via
//! [`crate::save`]. Bitbucket has no device flow and stays PAT-only.
...
fn parse_poll_response(body: &str) -> Result<PollOutcome> {
    ...
    if let Some(token) = response.access_token {
        if token.is_empty() {
            return Err(anyhow!("github returned an empty access token"));
        }
...
pub async fn request_device_authorization(
    ...
    let response = post_form(GITHUB_DEVICE_CODE_URL, body, http).await?;
...
pub async fn poll_device_authorization_once(
    ...
    let response = post_form(GITHUB_ACCESS_TOKEN_URL, body, http).await?;
