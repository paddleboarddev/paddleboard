...
#[derive(Default)]
pub struct FollowableViewRegistry(HashMap<TypeId, fn(&AnyView) -> Box<dyn FollowableItemHandle>>);
...
impl FollowableViewRegistry {
    pub fn register<I: FollowableItem>(cx: &mut App) {
        cx.default_global::<Self>()
            .0
            .insert(TypeId::of::<I>(), |view| {
                Box::new(view.clone().downcast::<I>().unwrap())
            });
    }
    ...
    pub fn to_followable_view(
        ...
        let view = view.into();
        let to_followable = this.0.get(&view.entity_type())?;
        Some(to_followable(&view))
    }
}
...
impl WorkspaceStore {
    pub fn new(client: Arc<Client>, _cx: &mut Context<Self>) -> Self {
        Self {
