    ...
    pub fn to_uri(&self) -> Url {
        match self {
            ...
            MentionUri::PastedImage { name } => {
                let mut url = Url::parse("paddleboard:///agent/pasted-image").unwrap();
            ...
            MentionUri::Selection {
                ...
                let mut url = if let Some(path) = abs_path {
                    ...
                    let mut url = Url::parse("paddleboard:///").unwrap();
            ...
            MentionUri::Rule { id, name } => {
                // PaddleBoard: emit paddleboard:// instead of zed://
                let mut url = Url::parse("paddleboard:///").unwrap();
                let rule_id = id
                    .get("User")
            ...
            MentionUri::TerminalSelection { line_count } => {
                let mut url = Url::parse("paddleboard:///agent/terminal-selection").unwrap();
                url.query_pairs_mut()
                    .append_pair("lines", &line_count.to_string());
                url
            }
            MentionUri::GitDiff { base_ref } => {
                let mut url = Url::parse("paddleboard:///agent/git-diff").unwrap();
                url.query_pairs_mut().append_pair("base", base_ref);
                url
            }
            MentionUri::MergeConflict { file_path } => {
                let mut url = Url::parse("paddleboard:///agent/merge-conflict").unwrap();
...
mod tests {
    ...
    fn test_parse_untitled_selection_uri() {
        let selection_uri = uri!("paddleboard:///agent/untitled-buffer#L1:10");
        let parsed = MentionUri::parse(selection_uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Selection {
                abs_path: None,
    ...
    fn test_parse_thread_uri() {
        let thread_uri = "paddleboard:///agent/thread/session123?name=Thread+name";
    ...
    fn test_parse_legacy_rule_uri() {
        ...
        // PaddleBoard: legacy zed:// mentions re-emit under the paddleboard:// scheme.
        assert_eq!(
            parsed.to_uri().to_string(),
            "paddleboard:///agent/rule/d8694ff2-90d5-4b6f-be33-33c1763acd52?name=Some+rule"
        );
    }
    ...
    fn test_parse_diagnostics_uri() {
        let uri = "paddleboard:///agent/diagnostics?include_warnings=true";
    ...
    fn test_parse_diagnostics_uri_warnings_only() {
        let uri = "paddleboard:///agent/diagnostics?include_warnings=true&include_errors=false";
        let parsed = MentionUri::parse(uri, PathStyle::local()).unwrap();
        match &parsed {
            MentionUri::Diagnostics {
                include_errors,
    ...
    fn test_invalid_zed_path() {
        assert!(MentionUri::parse("paddleboard:///invalid/path", PathStyle::local()).is_err());
        assert!(MentionUri::parse("paddleboard:///agent/unknown/test", PathStyle::local()).is_err());
    }
    ...
    fn test_parse_terminal_selection_uri() {
        let terminal_uri = "paddleboard:///agent/terminal-selection?lines=42";
        ...
        // Test single line
        let single_line_uri = "paddleboard:///agent/terminal-selection?lines=1";
