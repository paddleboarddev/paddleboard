...
use crate::{
    ...
    VariableName, PADDLEBOARD_VARIABLE_NAME_PREFIX, serde_helpers::non_empty_string_vec,
};
...
impl TaskTemplate {
    ...
    /// Validates that all `$PADDLEBOARD_*` variables used in this template are known
    /// variable names, returning a vector with all of the unique unknown
    /// variables.
    ///
    /// Note that `$PADDLEBOARD_CUSTOM_*` variables are never considered to be invalid
    ...
    fn collect_unknown_variables(template: &str, unknown: &mut HashSet<String>) {
        shellexpand::env_with_context_no_errors(template, |variable| {
            ...
            // separated by a `:`, for example, `${PADDLEBOARD_FILE:default_value} so we
            ...
            if variable_name.starts_with(PADDLEBOARD_VARIABLE_NAME_PREFIX)
                && let without_prefix = &variable_name[PADDLEBOARD_VARIABLE_NAME_PREFIX.len()..]
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        } else if variable_name.starts_with(PADDLEBOARD_VARIABLE_NAME_PREFIX) {
...
mod tests {
    ...
    fn test_errors_on_missing_zed_variable() {
        let task = TaskTemplate {
            label: "My task".into(),
            command: "echo".into(),
            args: vec!["$PADDLEBOARD_VARIABLE".into()],
            ..TaskTemplate::default()
        };
        assert!(
    ...
    fn test_variable_default_values() {
        let task_with_defaults = TaskTemplate {
            ...
            args: vec![
                "${PADDLEBOARD_MISSING_VAR:default_value}".to_string(),
                format!("${{{}}}", VariableName::Row.to_string() + ":42"),
            ],
            ..TaskTemplate::default()
        };

        // Test 1: When PADDLEBOARD_FILE exists, should use actual value and ignore default
        ...
        assert_eq!(
            ...
            "echo actual_file.rs",
            "Should use actual PADDLEBOARD_FILE value, not default"
        );
        assert_eq!(
        ...
        // Test 2: When PADDLEBOARD_FILE doesn't exist, should use default value
        ...
        assert_eq!(
            resolved.resolved.command.unwrap(),
            "echo fallback.txt",
            "Should use default value when PADDLEBOARD_FILE is missing"
        );
        assert_eq!(
            resolved.resolved.args,
        ...
        let task_no_default = TaskTemplate {
            ...
            command: "${PADDLEBOARD_MISSING_NO_DEFAULT}".to_string(),
            ..TaskTemplate::default()
        };
    ...
    fn test_unknown_variables() {
        // Variable names starting with `PADDLEBOARD_` that are not valid should be
        // reported.
        let label = "test unknown variables".to_string();
        let command = "$PADDLEBOARD_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        assert_eq!(task.unknown_variables(), vec!["PADDLEBOARD_UNKNOWN".to_string()]);

        // Variable names starting with `PADDLEBOARD_CUSTOM_` should never be reported,
        // as those are dynamically provided by extensions.
        let label = "test custom variables".to_string();
        let command = "$PADDLEBOARD_CUSTOM_UNKNOWN".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "${PADDLEBOARD_UNKNOWN:default_value}".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
        ...
        let label = "test custom variables".to_string();
        let command = "$PADDLEBOARD_FILE".to_string();
        let task = TaskTemplate {
            label,
            command,
            ..TaskTemplate::default()
