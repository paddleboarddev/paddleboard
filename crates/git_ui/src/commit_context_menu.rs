...
// PaddleBoard: widened from pub(crate) — git_graph is a separate crate in this fork.
pub struct CommitContextMenuData {
    pub sha: Oid,
    pub tag_names: Vec<SharedString>,
}

#[derive(Clone, Copy, PartialEq, Eq)]
pub enum CommitContextMenuSource {
    GitGraph,
    GitPanel,
}
            ...
            .when(source == CommitContextMenuSource::GitPanel, |menu| {
                menu.entry("Show in Git Graph", None, move |window, cx| {
                    window.dispatch_action(
                        // PaddleBoard: git_graph is its own crate here; the action
                        // is defined in git_panel (see the note there).
