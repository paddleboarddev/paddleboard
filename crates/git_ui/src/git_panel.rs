...
// PaddleBoard: actions for the Git Graph history view (which lives in the
// `paddleboard_git_graph` crate). Defined here in git_panel so that crate can
// reference `git_ui::git_panel::{Open, OpenAtCommit}` — this is the v0.1.4
// layout. Do NOT also adopt upstream's `git_ui::git_graph` module: it
// re-registers the same action names and newer gpui panics on duplicates.
//
// The `git_graph` namespace below is the USER-FACING action name (`git_graph::Open`
// in keymaps and the command palette). It deliberately did not change when the
// crate was renamed to `paddleboard_git_graph` — renaming it would break every
// existing keymap binding.
actions!(
    git_graph,
    [
...
fn git_panel_context_menu(
    ...
    ContextMenu::build(window, cx, |context_menu, _, _| {
        context_menu
            ...
            .action("View Stash", paddleboard_actions::git::ViewStash.boxed_clone())
                ...
                match (style, is_push) {
                    ...
                    (Toast | ToastWithLog { .. }, true) => {
                        ...
                        this.action("Create Pull Request", move |window, cx| {
                            window
                                .dispatch_action(Box::new(paddleboard_actions::git::CreatePullRequest), cx);
                        })
                    }
                    (Toast, false) => this,
                        ...
                        h_flex()
                            ...
                            .child(
                                div()
                                    ...
                                    .flex_grow_1()
                                    .cursor_text()
                                    .on_action(|&paddleboard_actions::editor::MoveUp, _, cx| {
                                        cx.stop_propagation();
                                    })
                                    .on_action(|&paddleboard_actions::editor::MoveDown, _, cx| {
                                        cx.stop_propagation();
                                    })
...
impl RenderOnce for PanelRepoFooter {
    fn render(self, _window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let branch_selector_button = Button::new("branch-selector", branch_name)
            ...
            .truncate(true)
            .on_click(|_, window, cx| {
                window.dispatch_action(paddleboard_actions::git::Switch.boxed_clone(), cx);
            });
        ...
        let branch_selector = PopoverMenu::new("popover-button")
            ...
            .trigger_with_tooltip(
                ...
                Tooltip::for_action_title("Switch Branch", &paddleboard_actions::git::Switch),
            )
            .anchor(Anchor::BottomLeft)
            .offset(gpui::Point {
                x: px(0.0),
