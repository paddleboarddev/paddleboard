...
impl ExtensionsPage {
    ...
    // PaddleBoard: upstream deleted `buttons_for_entry` and `ExtensionCardButtons`
    // in this merge, moving the card into `components/extension_card.rs`. The
    // function is gone rather than kept: nothing calls it, and the struct it
    // returns no longer exists anywhere in the workspace. Its one PaddleBoard
    // divergence — the rebranded incompatibility notice — was re-applied at the
    // new site instead.
