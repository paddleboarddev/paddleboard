...
use paddleboard_manifest::ManifestPanel;
use client::zed_urls;
...
pub fn init(cx: &mut App) {
    ...
    cx.on_action(|_: &paddleboard_actions::OpenLicenses, cx| {
        with_active_or_new_workspace(cx, |workspace, window, cx| {
            open_bundled_file(
                workspace,
...
pub fn initialize_workspace(app_state: Arc<AppState>, cx: &mut App) {
    ...
    cx.observe_new(move |workspace: &mut Workspace, window, cx| {
        ...
        let sandbox_status_item =
            cx.new(|cx| paddleboard_sandbox_prereqs_ui::SandboxStatusItem::new(workspace, cx));
...
fn initialize_panels(window: &mut Window, cx: &mut Context<Workspace>) -> Task<anyhow::Result<()>> {
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        futures::join!(
            ...
            add_panel_when_ready(manifest_panel, workspace_handle.clone(), cx.clone()),
...
fn open_about_window(cx: &mut App) {
    ...
    cx.open_window(
        WindowOptions {
            titlebar: Some(TitlebarOptions {
                title: Some("About PaddleBoard".into()),
                appears_transparent: true,
...
fn open_worktree_setup_tasks_file(
    workspace: &mut Workspace,
    _: &paddleboard_actions::OpenWorktreeSetupTasks,
    window: &mut Window,
    cx: &mut Context<Workspace>,
) {
    ...
    // Runs automatically after PaddleBoard creates a new git worktree.
    // $PADDLEBOARD_WORKTREE_ROOT is the new worktree's root directory, and
    // $PADDLEBOARD_MAIN_GIT_WORKTREE is the original repository's working directory.
    "label": "Set up new worktree",
    "command": "echo \"Setting up $PADDLEBOARD_WORKTREE_ROOT — edit this command\"",
    "cwd": "$PADDLEBOARD_WORKTREE_ROOT",
...
mod tests {
    ...
    fn test_bundled_settings_and_themes(cx: &mut App) {
        ...
        for theme_name in themes.list().into_iter().map(|meta| meta.name) {
            ...
            // Checks the compiled-in fallback theme (theme::DEFAULT_DARK_THEME),
            // which is loaded under JustBase — not the bundled-JSON default.
    ...
    async fn test_disable_ai_filters_keybindings(cx: &mut gpui::TestAppContext) {
        ...
        let user_binding = KeyBinding::new(
            "ctrl-enter",
            paddleboard_actions::assistant::InlineAssist { prompt: None },
            None,
        );
