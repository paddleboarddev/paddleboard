...
use paddleboard_actions::search::ToggleIncludeIgnored;
...
impl FileFinderDelegate {
    ...
    fn set_search_matches(
        ...
        if search_id >= self.latest_search_id {
            ...
            // PaddleBoard: upstream's channel-match block is dropped — it reads
            // `self.channel_store` and builds `Match::Channel`, both of which are
            // collaboration features this fork removed. `query_path` below is
            // still required by the worktree resolution that follows.
...
impl PickerDelegate for FileFinderDelegate {
    ...
    // PaddleBoard: the old inline body here carried our "create new" Plus
    // affordance. Upstream's render_match_impl now renders that Plus in the
    // start slot itself, so the fork patch retires rather than duplicating it.
