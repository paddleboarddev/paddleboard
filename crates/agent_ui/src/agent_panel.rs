    ...
    cx.observe_new(
        |workspace: &mut Workspace, _window, _cx: &mut Context<Workspace>| {
            workspace
                ...
                .register_action(|workspace, _: &ManageSkills, window, cx| {
                    if let Some(panel) = workspace.panel::<AgentPanel>(cx) {
                        workspace.focus_panel::<AgentPanel>(window, cx);
                        panel.update(cx, |panel, cx| {
                            // PaddleBoard: ManageSkills is a unit action; our
                            // deploy_rules_library still takes the richer
                            // OpenRulesLibrary payload.
                            panel.deploy_rules_library(
        ...
        if let BaseView::AgentThread { conversation_view } = &self.base_view {
            if conversation_view.entity_id() == draft.entity_id() {
                ...
                if self.overlay_view.is_some() {
                    ...
                    // Upstream renamed this call in the same merge; the
                    // surface-aware dispatch now lives in
                    // `activation_focus_handle`, so behaviour is unchanged.
...
impl agent::SiblingThreadHost for AgentPanelSiblingHost {
    fn create_sibling_thread(
        ...
        cx.spawn(async move |cx| {
            let agent_choice = match request.agent_id.as_deref() {
                None => None,
                Some(id) if id == agent::PADDLEBOARD_AGENT_ID.as_ref() => Some(Agent::NativeAgent),
                Some(id) => {
            ...
            let target_panel = if request.use_new_worktree {
                ...
                let branch_target = match request.base_ref.as_ref() {
                    Some(ref_name) => paddleboard_actions::NewWorktreeBranchTarget::ExistingBranch {
                        name: ref_name.clone(),
                    },
                    None => paddleboard_actions::NewWorktreeBranchTarget::CurrentBranch,
                };
                let action = paddleboard_actions::CreateWorktree {
    ...
    fn list_available_agents(&self, cx: &mut App) -> Result<agent::AvailableAgents> {
        ...
        agents.push(agent::AvailableAgent {
            id: agent::PADDLEBOARD_AGENT_ID.to_string(),
...
impl Focusable for AgentPanel {
    // PaddleBoard: upstream's simple version is taken. The surface dispatch that
    // used to live here moved to `Panel::activation_focus_handle` in this merge,
    // extended there with this fork's `Configuration` and `History` surfaces —
    // so nothing is lost, and the two methods no longer disagree about which
    // handle the panel reports.
...
impl Panel for AgentPanel {
    ...
    fn activation_focus_handle(&self, cx: &App) -> FocusHandle {
        match self.visible_surface() {
            ...
            // PaddleBoard: `Configuration` and `History` are this fork's surfaces,
            // so upstream's match doesn't cover them. Their dispatch used to live
            // in `Focusable::focus_handle`; upstream moved surface-awareness here
            // in this merge, which is where it now belongs.
...
mod tests {
    ...
    // PaddleBoard regression test: NewThread must produce a connected thread
    // even when the PromptStore fails to initialize (e.g. LMDB permission
    // error). Before the fix, `NativeAgentServer::connect` propagated the
    // PromptStore error with `?`, which put every new ConversationView into
    // LoadError and made it impossible to start any native-agent thread.
    #[gpui::test]
