...
// PaddleBoard: orchestration panel, MCP status/servers UI, and the status-bar usage gauge.
...
actions!(
    ...
    [
        ...
        ExpandMessageEditor,
        // PaddleBoard: these actions remain in use by PB surfaces (thread history
        // view, MCP servers view, in-panel agent configuration) after upstream
        // moved its config UI into settings_ui and dropped them.
...
pub fn init(
    ...
    // PaddleBoard: the Rules Library is kept (upstream migrated rules to skills);
    // `rules_library::init` calls `prompt_store::init` internally.
...
fn update_command_palette_filter(cx: &mut App) {
    ...
    CommandPaletteFilter::update_global(cx, |filter, _| {
        ...
        let skill_creator_actions = [
            TypeId::of::<paddleboard_actions::assistant::OpenSkillCreator>(),
            TypeId::of::<paddleboard_actions::assistant::CreateSkillFromUrl>(),
        ];
