...
static PADDLEBOARD_CLIENT_CHECKSUM_SEED: LazyLock<Option<Vec<u8>>> = LazyLock::new(|| {
    option_env!("PADDLEBOARD_CLIENT_CHECKSUM_SEED")
        ...
        .or_else(|| {
            env::var("PADDLEBOARD_CLIENT_CHECKSUM_SEED")
                .ok()
                .map(|s| s.as_bytes().into())
        })
});
...
pub static MINIDUMP_ENDPOINT: LazyLock<Option<String>> = LazyLock::new(|| {
    option_env!("PADDLEBOARD_MINIDUMP_ENDPOINT")
        .map(str::to_string)
        .or_else(|| env::var("PADDLEBOARD_MINIDUMP_ENDPOINT").ok())
});
...
impl Telemetry {
    ...
    pub fn has_checksum_seed(&self) -> bool {
        PADDLEBOARD_CLIENT_CHECKSUM_SEED.is_some()
    }
...
pub fn calculate_json_checksum(json: &impl AsRef<[u8]>) -> Option<String> {
    let checksum_seed = PADDLEBOARD_CLIENT_CHECKSUM_SEED.as_ref()?;
