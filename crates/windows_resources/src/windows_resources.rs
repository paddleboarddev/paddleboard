...
pub fn compile(manifest: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // PaddleBoard: app rename — Windows resource product names
    let (icon_filename, product_name) = match channel {
        "stable" => ("app-icon.ico", "PaddleBoard"),
        "preview" => ("app-icon-preview.ico", "PaddleBoard Preview"),
        "nightly" => ("app-icon-nightly.ico", "PaddleBoard Nightly"),
        _ => ("app-icon-dev.ico", "PaddleBoard Dev"),
    };
