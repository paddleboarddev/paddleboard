...
#[cfg(windows)]
impl MasterProcess {
    const CONNECTION_ESTABLISHED_MAGIC: &str = "PADDLEBOARD_SSH_CONNECTION_ESTABLISHED";
...
impl RemoteConnection for SshRemoteConnection {
    ...
    fn start_proxy(
        ...
        const VARS: [&str; 3] = ["RUST_LOG", "RUST_BACKTRACE", "PADDLEBOARD_GENERATE_MINIDUMPS"];
...
impl SshRemoteConnection {
    ...
    async fn ensure_server_binary(
        ...
        let wanted_version = cx.update(|cx| match release_channel {
            ...
            ReleaseChannel::Dev => {
                anyhow::bail!(
                    "PADDLEBOARD_BUILD_REMOTE_SERVER is not set and no remote server exists at ({:?})",
                    dst_path
                )
            }
