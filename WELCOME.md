...
- Installed items show a green badge; missing ones show an **Install / Sign In / Set Up / Learn More** action that does the right thing for the category — registry agent installs are a one-click settings write, CLI-based agents (like Google ADK) show a **Set Up** button that opens a terminal with the install command, sign-in flows route to your existing identity, MCP server adds delegate to the existing setup machinery, and bundled skills (`/test`, `/build-mcp`) install with **Add to project** / **Add to user** buttons that drop a markdown file into the right `.claude/commands/` directory, alongside built-in commands (`/verify`, `/review`, `/security-review`) that the agent already provides.
...
Vibe-coded something worth showing off? **`set sail: Deploy`** (command palette, or click the **⛵ sailboat in the status bar**) gives you serverless-first deploys of the current project to **[Cloud Run](https://cloud.run), [AWS Lambda](https://aws.amazon.com/pm/lambda), [Vercel](https://vercel.com), [Azure](https://azure.microsoft.com/products/container-apps) (Container Apps), [Cloudflare](https://workers.cloudflare.com) (Workers), or [Netlify](https://www.netlify.com)** — no YAML safari required.
...
- **Custom target:** not on the list? Pick **Custom target** under the platform grid and describe where you host — Knative or KEDA on your own Kubernetes cluster, Fly.io, a VPS with Docker — and the agent works from your description (plus any skills already in the project), asking for the specifics it's missing instead of guessing.
- **Rig the pipeline:** flip the mode toggle at the top of the modal and, instead of a one-off deploy, the agent does the vendor-agnostic CI/CD groundwork — a least-privilege deploy identity (keyless where the platform supports it), the target resource, and the exact deploy command — then offers to scaffold a starter config for *your* CI tool (GitHub Actions, GitLab, Jenkins, Buildkite, …) with every secret left as a labeled placeholder. You own the tool choice and all credentials; the agent never touches real secrets. Available today for Cloud Run, AWS Lambda, and Vercel; the newer platforms' pipeline packs are on the way.
...
- **Personas for sub-agents:** delegated work can wear its own identity — the agent can pass `persona` to its `spawn_agent` tool (or you can ask it to: "have a QA-persona sub-agent review this"), so a review pass genuinely thinks like a reviewer, not like the implementer grading their own work. The same works for container-isolated [Scion agents](#scion-agents): the `spawn_scion_agent` tool takes a `persona` too, and the `scion: Start Agent` modal has a persona selector — since a Scion agent runs outside your session, the persona's identity overlay rides the front of its task prompt.
...
**Delegating to Scion from an agent.** When the `scion` CLI is installed, agents gain a `spawn_scion_agent` tool. Instead of spawning an in-process sub-agent that shares your workspace, an agent can hand a well-scoped subtask to a Scion agent running in its own container and git worktree — true isolation for parallel work that writes to disk. The tool starts the agent, optionally waits for it to finish, returns the final phase plus a tail of its logs, and can `sync` the result back into your project when it completes. It also takes an optional `persona` (and the `scion: Start Agent` modal has a matching selector), so even an isolated agent can wear a QA-engineer or SRE identity — the persona overlay is prepended to its task prompt.
